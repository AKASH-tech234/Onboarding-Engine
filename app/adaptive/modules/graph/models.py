...
web-framework dependencies â€” Pydantic validation lives in the API layer.
...
class SkillNode:
    ...
    def __post_init__(self):
        ...
        if not (1 <= self.difficulty <= 5):
            raise ValueError(f"difficulty must be 1â€“5, got {self.difficulty}")
...
class SkillEdge:
    """
    A directed dependency edge: `from_id` â†’ `to_id` means
    `to_id` requires `from_id` as a prerequisite.
    """
    from_id:    str
    to_id:      str
    importance: Importance
    weight:     float   # 0.0â€“1.0; lower weight = breakable under time constraints

    def __post_init__(self):
        if self.from_id == self.to_id:
            raise ValueError(f"Self-loop on node '{self.from_id}'")
        if not (0.0 <= self.weight <= 1.0):
            raise ValueError(f"weight must be 0.0â€“1.0, got {self.weight}")
...
class CandidateSkillLevel:
    ...
    """
    canonical_id:    str
    candidate_level: float   # 0.0â€“1.0
    required_level:  float   # 0.0â€“1.0
...
class PrunedItem:
    ...
    Met skills never appear here â€” they are in PruneResult.pruned_ids.
    """
    skill_id:             str
    skill_label:          str
    domain:               str
...
class SubgraphResult:
    nodes:          dict[str, SkillNode]   # id â†’ node
