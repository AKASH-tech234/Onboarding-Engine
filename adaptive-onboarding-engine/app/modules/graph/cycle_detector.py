...
Two algorithms:
  1. detect()           — fast single-cycle detection via DFS 3-colour marking.
  2. find_all_cycles()  — find every cycle via NetworkX simple_cycles().
  3. find_edge_to_break() — pick the weakest edge in a cycle to remove.
  4. resolve_all_cycles() — iteratively break edges until the graph is a DAG.

All functions that modify graph structure operate on COPIES — the original
...
try:
    from graph.models import SkillEdge
    from utils.errors import CircularDependencyError
except ImportError:  # pragma: no cover - alternate package root
    from app.modules.graph.models import SkillEdge
    from app.utils.errors import CircularDependencyError
...
class _Colour(IntEnum):
    ...
    BLACK = 2   # fully processed — no unvisited successor


# ---------------------------------------------------------------------------
# 1. Single-cycle detection — fast, iterative DFS
# ---------------------------------------------------------------------------
...
def detect(g: nx.DiGraph) -> tuple[bool, list[str]]:
    ...
    Args:
        g: A NetworkX DiGraph (need not be the singleton — works on any DiGraph).
    ...
    parent  = {}                    # node → node (DFS tree parent)
...
def _reconstruct_cycle(from_id: str, to_id: str, parent: dict[str, str]) -> list[str]:
    """
    Reconstruct the cycle path given the back-edge from_id → to_id.
    ...
    # middle = [from_id, ..., child_of_to_id] — reverse for forward order
    middle.reverse()
...
def find_all_cycles(g: nx.DiGraph) -> list[list[str]]:
    ...
    # nx.simple_cycles returns [a, b, c] (open) — close each one
...
def find_edge_to_break(
    ...
    breakability score, defined as:
        ...
        score = weight × importance_multiplier
...
def resolve_all_cycles(g: nx.DiGraph) -> tuple[nx.DiGraph, list[dict]]:
    ...
    Operates on a COPY of `g` — the original is never mutated.
    ...
    for _ in range(max_iterations):
        ...
        if candidate is None:
            raise CircularDependencyError(
                f"Cannot resolve cycle [{' → '.join(cycle)}]. "
                "No breakable edge found — inspect graph-data source.",
                cycle=cycle,
            )
        ...
        logger.warning(
            "cycle_detector: broke edge %s → %s (weight=%.2f, importance=%s)",
