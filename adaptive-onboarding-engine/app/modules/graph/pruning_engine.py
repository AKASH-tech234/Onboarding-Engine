...
Two-pass transformation of the topologically sorted skill list:
  ...
  Pass 1 — CLASSIFY + COMPRESS
    For each skill in topo order:
      - Met  (delta ≤ GAP_THRESHOLD):   remove entirely (candidate already knows it)
      - Weak (GAP_THRESHOLD < delta ≤ WEAK_THRESHOLD):
              keep, but reduce effort = base_effort × delta
      - Missing (delta > WEAK_THRESHOLD):
              keep, full base effort (or 70% in fast_track mode)

  Pass 2 — TIME BUDGET TRIMMING
...
Thresholds:
  GAP_THRESHOLD  = 0.10  (delta ≤ 0.10 → met)
  WEAK_THRESHOLD = 0.30  (delta 0.10–0.30 → weak; delta > 0.30 → missing)
    ...
    *,
    ...
    for index, skill_id in enumerate(topo_order):
        ...
        if node is None:
            ...
            logger.warning(
                "pruning_engine: '%s' not found in graph at prune time — skipping",
                skill_id,
            )
        ...
        # this skill — it is only in the subgraph as a structural ancestor.
...
def compute_adjusted_effort(
    ...
    MISSING skills:
      deep_learning → full base effort
      fast_track    → 70% of base (crash-course assumption)

    WEAK skills:
      deep_learning → base × delta           (linear gap fraction)
      fast_track    → base × delta × 0.60    (review mode — steeper discount)

    All results are clamped to MIN_EFFORT (1 day).

    Args:
        base_effort_days: Base effort from the graph node.
        delta:            required_level − candidate_level (0.0–1.0).
...
def _apply_time_budget(
    ...
    Args:
        ...
        scores:      composite_score per skill (0–1).
    ...
    # Build in-subgraph dependent map: skill_id → set of IDs that depend on it
    dependents = _build_dependents_map(item_ids)

    # Sort by score ascending — lowest score trimmed first
    ...
    for candidate in candidates:
        ...
        if deps_in_retained:
            logger.debug(
                "pruning_engine: cannot trim '%s' — retained dependents: %s",
...
def _get_levels(
    ...
    not directly referenced in the JD), we conservatively assume:
      candidate_level = 0.0
      required_level  = implied from difficulty (0.4 for diff-1 → 0.8 for diff-5)
    """
    record = level_map.get(skill_id)
    if record is not None:
        return record.candidate_level, record.required_level

    # Skill is an ancestor pulled into the subgraph — not directly in the JD.
    # Conservative assumption: candidate knows nothing, moderate requirement.
    implied_required = round(0.4 + (difficulty - 1) * 0.1, 2)   # diff 1→0.4, diff 5→0.8
    return 0.0, implied_required


def _build_dependents_map(node_ids: set[str]) -> dict[str, set[str]]:
    """
    Build a map of skill_id → {skill IDs that depend on it} restricted to `node_ids`.
