...
Three-stage process:
  ...
  Stage 1 — Critical level bucketing (HARD constraint)
    ...
    dependency at critical level ≥ N. This is the fundamental ordering
    correctness guarantee.

  Stage 2 — Domain clustering (soft heuristic)
  ...
  Stage 3 — Phase packing (capacity constraints)
...
validate_phase_order() performs a post-hoc correctness check:
  For every edge (prereq → dependent) in the subgraph, verify that
...
try:
    from graph.models import LearningMode, Phase, PhaseValidationResult, PrunedItem, SkillEdge
    from graph.topological_sort import compute_critical_levels
except ImportError:  # pragma: no cover - alternate package root
    from app.modules.graph.models import LearningMode, Phase, PhaseValidationResult, PrunedItem, SkillEdge
    from app.modules.graph.topological_sort import compute_critical_levels
...
def assign_phases(
    ...
    *,
    ...
    # ── Stage 1: compute critical levels and annotate items
    ...
    # ── Stage 2: group by critical level
    level_groups = _group_by_critical_level(annotated)

    # ── Stage 2b: domain clustering within each group
...
def validate_phase_order(
    ...
    """
    # Build skill → phase_number lookup
...
def _pack_into_phases(
    ...
    A new phase opens when:
      ...
      - Next item is at a strictly HIGHER critical level than any item in the
        current phase (hard dependency boundary — always triggers a new phase)

    Single-skill phases are allowed: a skill that exceeds max_days by itself
    still forms its own phase (we never drop a skill to fit a budget here —
