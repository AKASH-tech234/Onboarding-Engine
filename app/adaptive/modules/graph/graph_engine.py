...
try:
    from app.adaptive.modules.graph.models import Importance, SkillEdge, SkillNode
    from app.adaptive.utils.errors import GraphLoadError, GraphNotLoadedError
...
class GraphEngine:
    ...
    def _default_graph_path() -> Path:
        ...
        primary = repo_root / "graph_data" / "base_graph.v1.json"
        if primary.exists():
            return primary

        legacy = repo_root / "adaptive-onboarding-engine" / "graph_data" / "base_graph.v1.json"
        if legacy.exists():
            return legacy
