...
The output is handed directly to topological_sort â†’ pruning_engine â†’ phase_assigner.

Key design decisions:
  - BFS (not recursive DFS) to avoid Python's default recursion limit on
    deep prerequisite chains (e.g. aws â†’ cloud_basics â†’ networking â†’ linux).
  - Deduplication via `visited` set â€” overlapping chains between gap skills
  ...
  - `importance_threshold` filter: fast_track mode sets threshold=0.8 to
    skip "recommended" prerequisites (weight â‰ˆ 0.5) and only follow
...
try:
    ...
    from app.adaptive.modules.graph.models import SkillEdge, SkillNode, SubgraphResult
...
def extract_subgraph(
    ...
    *,
    ...
    sub_edge_map: dict[tuple[str, str], SkillEdge] = {}   # (from, to) â†’ edge (dedup)
    unresolved:  list[str]          = []

    # BFS queue â€” start with deduplicated gap skills
    ...
    while queue:
        ...
        # â”€â”€ Resolve node: curated graph first, staged layer if allowed
        ...
        # â”€â”€ Collect prerequisite edges
        ...
        for edge in all_edges:
            ...
            if edge.weight < importance_threshold:
                logger.debug(
                    "subgraph_extractor: skipping edge %sâ†’%s (weight=%.2f < threshold=%.2f)",
            ...
            # Enqueue predecessor for BFS (even if met â€” structural node needed)
            if edge.from_id not in visited:
                queue.append(edge.from_id)

    # â”€â”€ Remove edges that reference nodes excluded by the threshold filter
    valid_edges = [
...
def get_descendants(skill_id: str) -> set[str]:
    ...
    Delegates to NetworkX nx.descendants() â€” O(V + E).
    """
