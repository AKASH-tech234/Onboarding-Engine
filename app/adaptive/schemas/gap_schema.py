...
class GapCategory(str, Enum):
    ...
    Three-tier classification based on effective gap magnitude.
      ...
      MISSING â†’ gap > 0.40   Candidate lacks the skill or is far below target.
      WEAK    â†’ 0.10 < gap â‰¤ 0.40   Partial knowledge; needs targeted upskilling.
      STRONG  â†’ gap â‰¤ 0.10   Candidate meets or is near the requirement.
    """
    MISSING = "missing"
    WEAK    = "weak"
    STRONG  = "strong"
...
class GapSeverity(str, Enum):
    ...
    Used by scoring_engine to weight pathway item priority.
      ...
      CRITICAL  â†’ large gap on a high-importance skill
      HIGH      â†’ large gap on medium-importance OR medium gap on high-importance
      MEDIUM    â†’ moderate gap or moderate importance
      LOW       â†’ small gap or low-importance optional skill
    """
    CRITICAL = "critical"
    HIGH     = "high"
    MEDIUM   = "medium"
...
class SignalInfluence(str, Enum):
    """
    How signals (years_experience, recency, project_complexity, â€¦) adjusted
...
class SignalSummary:
    ...
    confidence:         float   # confidence in the score (0â€“1)
    signal_adjustment:  float   # total additive delta applied by signals
    effective_score:    float   # raw_score Ã— confidence + signal_adjustment
...
class GapItem:
    ...
    effective_candidate Candidate's adjusted proficiency (0â€“1).
    effective_required  Job's effective requirement (0â€“1) after importance weighting.
    raw_gap             effective_required âˆ’ effective_candidate (clamped â‰¥ 0).
    importance          Original importance weight from the job profile (0â€“1).
    required_score      Original required_score from the job profile (0â€“1).
    ...
    notes               Free-text list â€” populated by higher layers (reasoning trace etc.).
    """
    canonical_id:        str
    label:               str
...
class GapAnalysis:
    ...
    items               All GapItem records â€” one per required/preferred skill.
    ...
    unmatched_ids       Skill IDs present on candidate profile but not required by job
                        (not gaps â€” used for "extra skills" display).
    overall_gap_score   Weighted aggregate gap across all required skills (0â€“1).
                        0 = perfect match, 1 = all skills entirely missing.
    pathway_type        "targeted" if overall_gap_score â‰¤ 0.60,
